...
app.config.update(
    ...
    SECRET_KEY='him@psql1',
    SQLALCHEMY_DATABASE_URI='postgresql://postgres:him@psql1@localhost/catalog_db',
...
#now we don't have to use query string ...
# this is a much more elegent way as url looks simpler and cleaner
@app.route('/user')
@app.route('/user/<name>')
def no_query_strings(name='Himanshu'):
    return '<h1> hello there! {}</h1>'.format(name)
# now we will learn how to work with different DATA TYPES
#STRING
@app.route('/test/<string:name>')
def work_string(name):
    return '</h1> Here is a String: '+name+'</h1>'
#numbers
@app.route('/add/<int:num1>/<int:num2>')
def work_int_add(num1,num2):
    return '<h1> The Sum is : {}'.format(num1+num2)+'</h1>'
#float
@app.route('/divide/<float:n1>/<float:n2>')
def division(n1,n2):
    return '<h1> The division is {}'.format(n1/n2)+'</h1>'
